...
var UpdaterFactory = NS('UpdaterFactory', NS('org.libspark.betweenJS.core.updaters::UpdaterFactory', Class.$extend({
    __classvars__:{
        ...
        getInstance:function(){
            return UpdaterFactory.instance || new UpdaterFactory() ;
        },
        toString:function()
        {
...
var DOMElementUpdater = NS('DOMElementUpdater', NS('org.libspark.betweenJS.core.updaters::DOMElementUpdater', AbstractUpdater.$extend({
    ...
    resolveValues:function(){
        ...
        for (key in source) {
            if (dest[key] === undefined) {
                dest[key] = (target.style[key] || '').replace(new RegExp(un[key]+'.*$'), '') ;
            }
            if (!!rMap['source.' + key]) {
                source[key] += (target.style[key] || '').replace(new RegExp(un[key]+'.*$'), '') ;
            }
        }
        for (key in dest) {
            if (source[key] === undefined) {
    ...
    updateObject:function(factor)
    {
        ...
        for (str in d) {
        		 try{
        		 	t.style[str] = parseFloat(s[str] * invert + d[str] * factor ) + this.units[str] ;
        		 }catch(e){
...
var EnterFrameTicker = NS('EnterFrameTicker', NS('org.libspark.betweenJS.tickers::EnterFrameTicker', Class.$extend({
	...
	render:function(){
	    ...
	    window.requestAnimationFrame(function(){eft.render()}) ;
	},
	stop:function()
    {
...
// EVENTS
var TweenEvent = NS('TweenEvent', NS('org.libspark.betweenJS.events::TweenEvent', IEvent.$extend({
   __classvars__:{
      version:'0.65',
      PLAY:'play',
      STOP:'stop',
...
/* EASINGS 
 * thanks to Robert Penner, of course*/
